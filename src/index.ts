...
import { openPromptFile } from "./api/open-prompt-file";
import { preprocessImage } from "./api/preprocess-image";
...
app.on("ready", async () => {
  ...
  ipcMain.handle("preprocess-image", async (event, buffer) => {
    return preprocessImage(buffer);
  });
  ipcMain.handle("open-prompt-file", openPromptFile);
  checkPermission();
  createWindow();
});
